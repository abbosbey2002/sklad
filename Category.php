...
if($_SERVER['REQUEST_METHOD'] == "POST" && isset($_POST['delete_ID'])){
  ...
  $id=$_POST['delete_ID'];
  echo $id;
  $cate_Obj->destroy($id, 'category');  
  header("Location: http://localhost:8000/Category.php");
}

if($_SERVER['REQUEST_METHOD'] == "POST" && !isset($_POST['delete_ID'])){
    $title=$_POST['title'];
    $cat_id=1;
    $cate_Obj->creat_category($cat_id, $title);
    header("Location: http://localhost:8000/Category.php");
}
...
<div class="main_section">
        ...
        <h2 class="h2 text text-center"> create category </h2>
        <input name="title" type="text" class="form-control" placeholder="category name">
